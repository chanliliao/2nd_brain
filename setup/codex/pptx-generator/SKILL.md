...
```
Read: .codex/skills/pptx-generator/brands/template/README.md
Read: .codex/skills/pptx-generator/brands/template/brand.json
Read: .codex/skills/pptx-generator/brands/template/config.json
```
...
mkdir -p .codex/skills/pptx-generator/.tmp

# Write script to temp directory
# (create file at .codex/skills/pptx-generator/.tmp/gen.py)

# Execute
uv run --with python-pptx==1.0.2 python .codex/skills/pptx-generator/.tmp/gen.py

# MANDATORY: Clean up immediately after execution
rm .codex/skills/pptx-generator/.tmp/gen.py
```
...
- Requires Python plus `uv` and `python-pptx==1.0.2` in the Codex runtime.
- If `uv` is unavailable, run equivalent Python scripts via the environment manager available in this repo.
- Keep outputs rooted in `.codex/skills/pptx-generator/` and preserve the same brand/layout contracts.
