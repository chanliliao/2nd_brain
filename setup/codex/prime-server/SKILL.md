...
1. Study the vertical feature slice (`src/features/polls/`) â€” models, schemas, repository, service, actions
2. Study the database setup (`src/core/database/`) â€” schema, client, migrations
...
- **Patterns**: Vertical slice (models â†’ schemas â†’ repository â†’ service â†’ actions), error classes with HTTP status codes
- **Server Actions**: How mutations flow from UI â†’ action â†’ service â†’ repository
