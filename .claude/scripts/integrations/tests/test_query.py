...
def test_dispatch_exception_prints_error_and_exits_1(capsys):
    ...
    with patch("integrations.github.cli_dispatch", side_effect=RuntimeError("auth failed")):
        with pytest.raises(SystemExit) as exc_info:
            main(["github", "prs"])
    assert exc_info.value.code == 1
    captured = capsys.readouterr()
...
def test_debug_flag_reraises_exception():
    ...
    with patch("integrations.github.cli_dispatch", side_effect=RuntimeError("boom")):
        with pytest.raises(RuntimeError, match="boom"):
            main(["github", "--debug", "prs"])
...
def test_multiple_args_passed_through():
    ...
    with patch("integrations.github.cli_dispatch") as mock_dispatch:
        main(["github", "diff", "owner/repo", "42"])
